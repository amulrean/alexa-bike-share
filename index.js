...
var appId = process.env.ALEXA_APP_ID; //'amzn1.echo-sdk-ams.app.your-skill-id';
var googleGeocodeApiKey = process.env.GOOGLE_GEOCODE_API_KEY;
...
var welcomeMessage =
  "Capital Bike Share Info. Say search and then a location or address to find station status for bike share docks near the location. Try Saying: search Clarendon Metro or find station near 1600 Pennsylvania Ave.";

var welcomeRepromt =
  "Try saying Search Clarendon Virginia or Search 1600 Pennsylvania Ave, Washington DC";
...
var nextOrPreviousMessage =
  "To hear information about other bike share stations in the area say next, previous, repeat or more information.";

var goodbyeMessage = "OK, have a good ride.";

var HelpMessage =
  "Here are some things you can say: A location or address like Clarendon Virginia or 1600 Pennsylvania Ave, Washington DC. What would you like to do?";
...
var newSessionHandlers = {
  ...
  LocationSearchIntent: function() {
    ...
    var searchValue = _.get(
      this,
      "event.request.intent.slots.admin.value",
      undefined
    );
  ...
  "AMAZON.NextIntent": function() {
    if (nearByStations && nearByStations.length > 0) {
      ...
      if (nearByStationIndex === nearByStations.length) {
        ...
        response +=
          "Station " + verbalIndex + " of " + nearByStations.length + " ";
...
function getCurrentBikeShareStationResponse() {
  var filledResponse = getBikeStationResponse(
    nearByStations[nearByStationIndex]
  );
...
function getBikeStationResponse(stationData) {
  return (
    stationData.name[0] +
    " has " +
    stationData.nbBikes[0] +
    " bikes available and " +
    stationData.nbEmptyDocks[0] +
    " empty docks. "
  );
}

function getLocalityLongName(geoCoderResult) {
  var localityRecord = geoCoderResult.address_components.find(function(
    addressComponent
  ) {
    return (
      addressComponent &&
...
function getStationsWithinDistance(location, distanceInMeters) {
  ...
  if (cachedBikeShareResults) {
    filtered = cachedBikeShareResults
      .map(function(el) {
        var coordinates = {
          latitude: parseFloat(el.lat[0]),
          longitude: parseFloat(el.long[0])
        };
...
function setNearByStations(geoCoderResults, distanceInMeters) {
  ...
  for (var resultKey in geoCoderResults) {
    ...
    var closeStations = getStationsWithinDistance(
      resultLocation,
      distanceInMeters
    );
...
function processGeoCoderResults(alexa, geoCoderResults, searchValue) {
  ...
  if (nearByStations && nearByStations.length > 0) {
    ...
    response +=
      "Say next station, more information or search for another location.";
